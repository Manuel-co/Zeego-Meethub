...
import { onAuthStateChanged } from 'firebase/auth'
import React, { useEffect, useState } from 'react'
import { firebaseAuth, meeetingsRef } from '../utils/firebaseConfig'
...
function JoinMeeting() {
    ...
    const getMeetingData = async () => {
      if(params.id && userLoaded) {
        ...
        if(fetchedMeetings.docs.length) {
          ...
          const isCreator = meeting.createdBy === user?.uid;
          if(meeting.meetingType==="1-on-1") {
            if(meeting.meetingUsers[0] === user?.uid || isCreator) {
              if(meeting.meetingDate === moment().format("L")){
                setIsAllowed(true);
              }
              else if (
              ...
              const index  = meeting.invitedUser.findIndex(
                (invitedUser: string) => invitedUser === user?.uid
              );
              if (index !== -1 || isCreator) {
                if(meeting.meetingDate === moment().format("L")){
  ...
  const appId = 1534144345;
  const serverSecret = "d6c3ed387f82bf08bc129f6d3c676353"; 
  ...
  return (
    <div>{isAllowed && (
      <div className="myCallContainer" ref={myMeeting} style={{
        width: "100%",
        height: "100vh"
      }}>
      </div>)}
